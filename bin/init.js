...
var isWindows=process.platform==="win32";var __dirname=function(){var x=dirname(decodeURI(new URL(import.meta.url).pathname));return resolve(isWindows?x.substring(1):x)}();var cwd=isWindows?"file:\\".concat(process.cwd()):relative(__dirname,process.cwd());

_asyncToGenerator(_regeneratorRuntime().mark(function _callee(){var _args$;var _process$argv,args,arg,isWindows,requires,alloweds,importing,_iterator,_step,require,gitignore,toIgnore,rebuilded,_yield$import,options;return _regeneratorRuntime().wrap(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:_process$argv=_toArray(process.argv),args=_process$argv.slice(2);arg=((_args$=args[0])===null||_args$===void 0?void 0:_args$.replace(/-/g,""))||"start";isWindows=process.platform==="win32";requires={dirs:[".library"],files:[".babelrc"]};alloweds={init:true,start:"../lib/tasks/start/index.js",build:"../lib/tasks/build/index.js",TEST:"../lib/tasks/start/index.js"};if(!(arg!=="TEST"&&!alloweds[arg])){_context.next=8;break}console.error("Command \"".concat(arg,"\" not found.").concat(EOL,"Use \"init\", \"start\" or \"build\".").concat(EOL));return _context.abrupt("return");case 8:importing=new draft("Importing required local modules: ".concat(sh$1.green).concat(sh$1.dim,"[ ").concat(sh$1.italic,"autoprefixer, babel, postcss, sass and uglifyjs").concat(sh$1.reset).concat(sh$1.green).concat(sh$1.dim," ]"));_iterator=_createForOfIteratorHelper(requires.dirs);_context.prev=10;_iterator.s();case 12:if((_step=_iterator.n()).done){_context.next=23;break}require=_step.value;if(!isWindows){_context.next=19;break}_context.next=17;return exec("xcopy "+normalize("".concat(__dirname,"/../").concat(require,"\\"))+" "+normalize("./".concat(require,"\\"))+" /s /e");case 17:_context.next=21;break;case 19:_context.next=21;return exec("cp -r "+normalize("".concat(__dirname,"/../").concat(require))+" "+normalize("./".concat(require)));case 21:_context.next=12;break;case 23:_context.next=28;break;case 25:_context.prev=25;_context.t0=_context["catch"](10);_iterator.e(_context.t0);case 28:_context.prev=28;_iterator.f();return _context.finish(28);case 31:requires.files.forEach(function(require){if(!fs.existsSync(normalize("./".concat(require))))fs.copyFileSync(normalize("".concat(__dirname,"/../").concat(require)),normalize("./".concat(require)));});if(fs.existsSync(normalize("./package.json"))){_context.next=36;break}fs.copyFileSync(normalize("".concat(__dirname,"/../.github/workflows/resources/_package.json")),normalize("./package.json"));_context.next=36;return exec("npm i");case 36:if(!fs.existsSync(normalize("./.swrc.js")))fs.copyFileSync(normalize("".concat(__dirname,"/../.github/workflows/resources/_swrc.js")),normalize("./.swrc.js"));if(!fs.existsSync(normalize("./.gitignore")))fs.copyFileSync(normalize("".concat(__dirname,"/../.github/workflows/resources/_gitignore")),normalize("./.gitignore"));else {gitignore=fs.readFileSync(normalize("./.gitignore"),"utf-8");toIgnore=["dist","release","src/exit",".library/addEventListener",".library/selector",".library/package.json","node_modules","package-lock.json","yarn.lock"];toIgnore.forEach(function(ignore){var regex=RegExp(ignore,"gm");if(!regex.test(gitignore))gitignore+="".concat(EOL).concat(ignore);});fs.writeFileSync(normalize("./.gitignore"),gitignore);}_context.next=40;return rebuildFiles(arg);case 40:rebuilded=_context.sent;importing.stop(1);if(rebuilded){_context.next=44;break}return _context.abrupt("return");case 44:_context.prev=44;if(!fs.existsSync("./.swrc.js")){_context.next=53;break}_context.next=48;return import('./config-f19c51c3.js');case 48:_yield$import=_context.sent;options=_yield$import.options;if(!(arg==="start"&&options!==null&&options!==void 0&&options.initalCommit&&!fs.existsSync("./.git"))){_context.next=53;break}_context.next=53;return exec("git init && git add . && git commit -m \"Initial Commit\"");case 53:_context.next=57;break;case 55:_context.prev=55;_context.t1=_context["catch"](44);case 57:if(!(typeof alloweds[arg]==="string")){_context.next=60;break}_context.next=60;return import(alloweds[arg]);case 60:args.includes("--TEST")&&console.log("PASSED");case 61:case"end":return _context.stop();}}},_callee,null,[[10,25,28,31],[44,55]])}))();
