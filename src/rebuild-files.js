...
const rebuildFiles = async (arg) => {
  ...
  try {
    ...
    if (!packageFile.scripts?.update)
      packageFile.scripts.update = 'npx npu; npm i --ignore-scripts';
    ...
    for (const dependence of dependencies) {
      if (
        ...
        if (compatibility.node && compatibility.dependencies?.[dependence]) {
          packageFile.devDependencies[dependence] =
            compatibility.dependencies[dependence];
        } else
          packageFile.devDependencies[dependence] = `^${await latest(
            dependence
          )}`;
