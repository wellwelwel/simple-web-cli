...
import fs from 'fs';
import { extname, join, resolve as normalize } from 'path';
...
(async () => {
        ...
        setTimeout(async () => {
          for (const expected in expecteds) {
            try {
              ...
              try {
                const resourcesPath = normalize(
                  './.github/workflows/resources/tests'
                );
                const destPath = normalize('./temp/src');

                await sh(
                  `cp "${join(resourcesPath, expected)}" "${join(
                    destPath,
                    expected
                  )}"`
                );
              ...
              await new Promise((resolve) => {
                ...
                const attemp = setInterval(async () => {
                  ...
                  if (
                    fs
                      .readFileSync(normalize(`./temp/dist/${file}`), 'utf-8')
                      ?.trim()?.length === 0
                  )
                    return;
              ...
              const compare = fs.readFileSync(
                normalize(`./temp/dist/${file}`),
                'utf-8'
              );

              console.log(
                copied && compare === output
                  ? `   \x1b[32m✔\x1b[0m`
                  : `   \x1b[31m✖\x1b[0m`,
                name
              );
  ...
  const expecteds = {
    ...
    'test-file.css': {
      name: 'Building CSS',
      output:
        'div{display:-webkit-box;display:-webkit-flex;display:-moz-box;display:flex}',
    },
    'test-file.scss': {
      name: 'Building SCSS',
      output:
