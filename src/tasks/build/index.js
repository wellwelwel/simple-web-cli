...
import { blacklist, build, source } from '../../modules/config.js';
...
(async () => {
  ...
  try {
    async function buildFiles() {
      ...
      const moreTypes =
        typesOver.length > 0 ? ` and ${typesOver.length} more` : '';
      ...
      if (blacklistCount > 0)
        console.log(
          `${sh.blue}ℹ ${sh.reset}${sh.bold}${blacklistCount}${sh.reset} file(s) in Blacklist`
        );
    }
    ...
    async function gerarDeploy() {
      ...
      try {
        ...
        loading.stop(
          1,
          `Successfully compressed into: ${sh.underscore}${sh.blue}${sh.bold}./${final}.zip`
        );
      } catch (error) {
    ...
    async function clearTemp() {
      ...
      if (fs.existsSync(`${source}${sep}exit`))
        fs.unlinkSync(`${source}${sep}exit`);
      if (fs.existsSync(to)) fs.rmSync(to, { recursive: true, force: true });
    ...
    loading.stop(
      1,
      `Finished in ${sh.green}${msToTime(performance.now() - startTime)}`
    );
  } catch (e) {
