...
async function processJS(file, local = false, replace = 'start') {
  ...
  try {
    ...
    const parents = Object.entries(deps).filter(([, imports]) => imports.includes(relFile)).map(([key]) => `${source}/${key}`);
    if (parents.length > 0) {
  ...
  for (let i = 0; i < segments.length; i++) {
    if (fs.existsSync(curSource)) {
      for (const item of fs.readdirSync(curSource)) {
        ...
        if (!fs.existsSync(linkPath)) fs.symlinkSync(nodePath.resolve(curSource, item), linkPath);
      }
    }
  ...
  try {
    async function buildFiles() {
      ...
      for (const file of files) {
        ...
        if (fileType === 'js' || fileType === 'ts') await processJS(file, to, 'build');else if (fileType === 'scss' || fileType === 'css') await processCSS(file, to, 'build');else {
